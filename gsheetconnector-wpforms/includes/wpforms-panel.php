...
class WPGS_FormBuilder {
    ...
    if(isset($googlesheet['gs_sheet_manuals_sheet_name']) && isset($googlesheet['gs_sheet_manuals_sheet_id']) && isset($googlesheet['gs_sheet_manuals_sheet_tab_name']) && isset($googlesheet['gs_sheet_manuals_sheet_tab_id'])){
     ...
     Please add header manually in google sheet.for the reference  <a target='_blank' href=$new_link>Click here.</a> 
		...
		$result .= "<div class='field-maps'>";
				...
				$result .= "<div class='wpforms-panel-field'>";	
					$result .= "<label>Form Field   <a style='color:#4AA94F;font-weight:bold;' href='https://www.gsheetconnector.com/wpforms-google-sheet-connector-pro'target='_blank'>Upgrade To Pro</a></label>";
				$result .= "</div>";
 ...
 // Get the earliest and latest dates from the wpforms_entries table
    ...
    margin: 10px 0;
    font-weight: bold; display: contents !important;'>" . __( 'From Date', 'wpforms-webhooks' ) . ":</label>";
            $result .= "<input type='date' id='sync-from-date' name='sync_from_date' value='$from_date' min='$from_date' max='$to_date' class='wpgs-date-picker' style='
    margin: 0 10px;
    padding: 5px 10px;'>";

            $result .= "<label for='sync-to-date' style='padding: 5px 0;
    margin: 10px 0;
    font-weight: bold; display: contents !important;'>" . __( 'To Date', 'wpforms-webhooks' ) . ":</label>";
