...
$wpforms_lite_check = 'wpforms-lite/wpforms.php';
$wpforms_free_check = 'wpforms/wpforms.php';
$wpforms_pro_check  = 'wpforms-pro/wpforms.php';

// Helper: get plugins active on one site in multisite
if (!function_exists('get_activated_plugins_for_site')) {
...
// Case 1: Multisite + Site-specific activation
if (is_multisite() && !empty($current_site_id)) {
   $active_plugins = get_activated_plugins_for_site($current_site_id);

   if (
      in_array($wpforms_lite_check, $active_plugins) ||
      in_array($wpforms_free_check, $active_plugins) ||
      in_array($wpforms_pro_check, $active_plugins)
   ) {
      $activate_the_plugin = true;
   }
}

// Case 2: Multisite Network Activated
if (is_multisite()) {
   $network_plugins = get_site_option('active_sitewide_plugins', []);

   if (
      array_key_exists($wpforms_lite_check, $network_plugins) ||
      array_key_exists($wpforms_free_check, $network_plugins) ||
      array_key_exists($wpforms_pro_check, $network_plugins)
   ) {
      $activate_the_plugin = true;
   }
}
...
define('WPFORMS_GOOGLESHEET_VERSION', '4.0.3');
define('WPFORMS_GOOGLESHEET_DB_VERSION', '4.0.3');
...
// 🎯 NOW load Freemius — ONLY if WPForms exists
if ($activate_the_plugin && !function_exists('gw_fs')) {
...
class WPforms_Gsheet_Connector_Init
{
   ...
   public function add_css_files()
   {
      if (is_admin() && (isset($_GET['page']) && ($_GET['page'] == 'wpform-google-sheet-config'))) {
         ...
         wp_enqueue_style('wpform-gs-connector-font', WPFORMS_GOOGLESHEET_URL . 'assets/css/fontawesome.css', WPFORMS_GOOGLESHEET_VERSION, true);
   ...
   {
      if (is_admin() && (isset($_GET['page']) && ($_GET['page'] == 'wpform-google-sheet-config'))) {
         wp_enqueue_script('wpform-gsconnectorjs', WPFORMS_GOOGLESHEET_URL . 'assets/js/wpform-gs-connector.js', WPFORMS_GOOGLESHEET_VERSION, true);
         ...
         wp_enqueue_script(
            ...
            WPFORMS_GOOGLESHEET_URL . 'assets/js/gs-connector-extensions.js',
            array('jquery'),
            WPFORMS_GOOGLESHEET_URL,
            true
         );
   ...
   public function get_wpforms_system_info()
   {
      ...
      $system_info .= '<table>';
      $system_info .= '<tr><td>Plugin Name</td><td>GSheetConnector For WPForms</td></tr>';
